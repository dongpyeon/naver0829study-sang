...
-- num에 null 값을 주면 null이 들어가는게 아니라 auto 숫자가 발생된다.
...
insert into sawon values (null,'이하나',120,'여자','홍보부');
-- Error Code: 3819. Check constraint 'ck_sawon_gender' is violated.
insert into sawon values (null,'이하나',120,'여','홍보부');

-- 제대로 된 데이터 각자 7개 정도 추가
insert into sawon values (null,'강호도',90,'남자','인사부');
...
select * from sawon order by name limit 3,3; -- 3번부터 3개만 조회

-- 문자 함수들 연습
select concat('Hello', 'Kitty', '!!') from dual;
-- sawon의 name에서 '이'가 포함한 데이터 조회
...
select instr('Happy Day', 'Day') from dual; -- Day의 위치 찾기
