...
<body>
	<c:set var="today" value="<%=new Date() %>"/> <!-- 자바 명령어를 부득이하게 불러오기 할때 사용 -->
	<h4>
	...
	<h4>${pathurl}</h4>
	<hr>
	<fmt:requestEncoding value="utf-8"/> <!-- post방식 한글이 깨질때 이걸 넣어주면 해결-->
	...
	<c:choose>
		...
		<c:when test="${param.nara=='캐나다'}">
			<h3 style="color:magenta">캐나다의 메이플시럽은 유명합니다.</h3>		
		</c:when>
		<c:when test="${param.nara=='미국'}">
			<h3 style="color:magenta">미국의 five napkin은 맛있습니다.</h3>		
		</c:when>
		<c:otherwise>
			<h3 style="color:gray">${param.nara}은 어떤나라인가요?</h3>		
		</c:otherwise>
	</c:choose>
	<hr>
