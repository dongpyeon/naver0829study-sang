...
public class Ex2_KeyInput {
	...
	public static void main(String[] args) {
		...
		// Date 클래스를 이용해서 현재 년도를 구해보자(jdk1.1에서 deprecate되었지만 여전히 많이 사용함)
		// 권장 : Calendar를 권장
		Date date=new Date();
		...
		//if문으로 할 경우
		String msg="";
		//지연변수는 자동초기화가 안되고 쓰레기값이 들어있스므로
		//값이 안들어가 경우를 대비해서 초기값을 지정하는것이 좋다.
		//지역변수는 그 구역을 빠져나가게되면 자동소멸되므로 사용이 불가능한다.
		if(age>=20) 
		{
			//한 문장만 쓸경우 {} 구역 설정 생략가능하지만 두문장 이상일경우는 반드시 구역설정({})
			msg="성인";
		}else {
			msg="미성년자";
		}
