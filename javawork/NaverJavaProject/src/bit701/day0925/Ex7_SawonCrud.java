...
public class Ex7_SawonCrud {
	...
	public void deleteSawon()
	{
		...
		try {
			...
			if (n==0)
				System.out.println("삭제할 사원이 존재 하지 않습니다..");
			else
	...
	public void updateSawon()
	{
		...
		System.out.println("수정할 사원 이름는?");
		String name=sc.nextLine();
		System.out.println("수정할 사원 점수는?");
		int score=Integer.parseInt(sc.nextLine());
		System.out.println("수정할 사원 부서는?");
		String buseo=sc.nextLine();
		...
		try {
			...
			int n=pstmt.executeUpdate();			
			if (n==0)
				System.out.println("해당 인원이 없습니다.");
			else
