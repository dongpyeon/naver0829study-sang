...
<script type="text/javascript">
	$(function(){
		...
		$(document).on("click","img.small",function(){
			let num=$(this).attr("num");
			//페이지 이동
			//location.href="myshopdetail.jsp?num="+num;
			//es6의 리터럴 문자 사용시 -jsp자체적으로 $기능이 있으므로
			//앞에 \를 붙여서 변수를 가져온다.
			//location.href="myshopdetail.jsp?num="+num;
			location.href=`myshopdetail.jsp?num=\${num}`;
		})
...
<div class="input-group" style="width:650px;">
  <h5 class="alert alert-danger" style="width:650px;">총 <%=list.size()%>개의 상품이 있습니다
  <button type="button" class="btn btn-outline-info btn-sm" style="float: right"
    onclick="location.href='myshopinform.jsp'">상품등록</button>
  </h5>  
</div>
<table class="table table-bordered shoptb" style="width:650px;">
	<thead>
		<tr>
			<th width="50">번호</th>
			<th width="150">상품명</th>
		...
		<tr>
			...
			{%>
				<tr align="center">
					<td><%=++n %></td>
					<td align="left">
						<img class="small" src="../image/shop/<%=dto.getPhoto() %>"
						num="<%=dto.getNum()%>">
						<%=dto.getSangpum() %>
					</td>
					<td>
